    ...
    return (req, res, next) => {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return res.status(401).json({
                error: { code: '401 NO_AUTORIZADO', message: 'Token de acceso requerido', details: [] },
        ...
        try {
            ...
            if (rolesPermitidos.length > 0 && !rolesPermitidos.includes(payload.rol)) {
                return res.status(403).json({
                    error: { code: '403 ACCESO_DENEGADO', message: 'No tiene permisos para esta operación', details: [] },
                    meta: { traceId: req.traceId || 'N/A', timestamp: new Date().toISOString(), path: req.path }
                })
            }
            ...
            return res.status(401).json({
                error: { code: '401 TOKEN_INVALIDO', message: 'Token inválido o expirado', details: [error.message] },
                meta: { traceId: req.traceId || 'N/A', timestamp: new Date().toISOString(), path: req.path }
            })
        }
    }
